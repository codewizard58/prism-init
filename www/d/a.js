...
{   var cmd;
    var ch = e.keyCode;
    var f;
    var fontsize = curfontsize;
    var msg="";

    if( ch == 13){
        cmd = cmdline.join("").substr(0, cmdpos);
    ...
    msg += cmd;
    msg += "</span>";
    f = document.getElementById("cmdin");
	if( f != null){
		f.innerHTML = msg;
	}
}

function UItouchstart(e)
{	
	if( !handletouch){
		setdisplayformat();
		handletouch = true;
...
//	}
	...
	return "font-size:"+Math.floor(ew)+"px;font-family:Courier;";
}
...
{	var msg="";
	...
	if( hidecompass){
		msg += '<div class="boxred" style="position:fixed;z-index:2;right:5px;top:15px;background-color:white;" >';
		msg += '<input type="button" value="Show" onclick="UIshowcompass();" /><br />';
		msg += '<input type="button" value="Edit" onclick="UIeditor();" />';
		msg += '</div>\n';
	}else {
		msg += '<div class="boxred" style="position:fixed;z-index:2;max-width:'+(mid-50)+'px;top:15px;left:'+(mid-10)+'px;background-color:white;" >';
			msg += "<table><tr><td valign='top'>\n";
			msg += '<div style="'+fontsize+';" id="title">\n';
			msg += "</div>\n";
			msg += '</td><td>';
			msg += '<input type="button" value="|^|" onclick="UIshowcompass();" /></td>\n';
			msg += '</tr><tr><td >\n';
			msg += '<div style="'+fontsize+';" id="commands">';
			msg += '</div>\n';
			msg += '</td></tr><tr><td >\n';
			msg += '<div style="'+fontsize+';" id="contents">';
			msg += '</div>\n';
			msg += '</td></tr><tr><td >\n';
			msg += '<div style="'+fontsize+';" id="inventory">';
			msg += '</div>\n';
			msg += "</td></tr></table>\n";
		msg += '</div>\n';
	}
...
// layout
function simplelayout(fontsize)
{	var msg="";
	var mid = Math.floor(screenwidth/2);

	msg += '<div style="'+fontsize+';width:'+(histwidth-10)+'px;" >';
	msg += '&nbsp;<br />\n';
	msg += '</div>\n';
	...
	msg += "<table><tr><td valign='top'>\n";
		...
		msg += "</div>\n";
	msg += '</td></tr><tr><td >\n';
	msg += '<div style="'+fontsize+';" id="history">';
	msg += '</div>\n';
		msg += '</td></tr><tr><td >\n';
		msg += '<div style="'+fontsize+';" id="commands">';
		msg += '</div>\n';
...
// layout
function splitlayout(fontsize)
{	var msg="";
	var mid = Math.floor(screenwidth/2);

	// split screen history=75%
	msg += "<table><tr><td valign='top'>\n";
	msg += '<div style="'+fontsize+';width:'+(histwidth-10)+'px;" id="history">';
	msg += '</div>\n';
	msg += "</td><td valign='top' width='"+(screenwidth-histwidth-10)+"px'>\n";
...
function compasslayout(fontsize)
{	var msg="";
...
{	var msg = "";
	...
	msg += '<div style="position:fixed;z-index:1;top:5px;left:5px;background-color:white;" id=cmdin>\n';
	if( handletouch){
	 	msg += '<span style="'+curfontsize+'" >Cmd: </span>';
	    msg += '<input type="text" id="touchin" onchange="UIdoinput();" onfocus="UIfocus();" size="20" style="'+curfontsize+'" />\n';
	}
	msg += "</div>\n";
...
// layout
// screen width or height changed.
//
...
{	var msg="";
	...
	// landscape
	  if( displaymode == 0){
		msg += splitlayout(fontsize);

	  }else if(displaymode == 1){
		msg += compasslayout(fontsize);
	  }else if(displaymode == 2){
		msg += simplelayout(fontsize);
	  }
	}else {
	// portrait
		msg += simplelayout(fontsize);
	...
	msg += '<div id="debugdiv" style="position:fixed;z-index:1;max-width:'+(mid-20)+'px;top:25px;left:'+(mid-60)+'px;background-color:white;">\n';
	msg += '<textarea id="debugtext" cols="80" rows="20" style="display:none;"></textarea>\n';
	msg += '</div>'+"\n";

	mw.innerHTML = msg;

	waitfordebugtext = true;
}
...
//	msg += '<div id="messages" style="'+this.fontsize+';">';            
//      msg += errormsg;
//      msg+= "</div>\n";

      msg+= '<table style="'+this.fontsize+'" class="box">';
      msg+= "    <tr>";
...
//////////////////////////////////////////////////////////////////////////////////
basic_room.prototype = Object.create(room.prototype);
...
function UIdoinput()
{	var di = document.getElementById("touchin");
    var cmd;
    var cmd2;
